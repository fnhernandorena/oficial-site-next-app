...
export default async function WorkExp() {
  ...
  return (
    ...
    {work?.map((work) =>
        <div key={work.id}>
    <h3 className="text-3xl font-bold text-sky-300 border-double border-b-8 border-r-8 border-sky-300 my-4">{work.tituloita}</h3>
    <p className="text-xl text-white my-5">{work.cuerpoita}</p>
    {work.link13 !== null && (
  <Link
